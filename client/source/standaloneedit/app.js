...
const ReducerGenericApp = function(in_state, in_action){
	...
	switch (in_action.type){
		case "toggleGeneric":
			...
			result.generic = (result.generic === false);
			return result;
		case "setUnits":
			var result = Object.assign({}, in_state);
...
const AppMapDispatch = function(in_dispatch, in_props){
	return {
		...
		"onToggleGeneric" : function(){ in_dispatch({"type":"toggleGeneric"}); },
...
in_props {
	...
	"specialise" : 
 */
const App = connect(AppMapState, AppMapDispatch)(function(in_props){
	return React.createElement(AppMenu, in_props);
});

export {AppReducer, App}