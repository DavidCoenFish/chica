...
module.exports.getFolderMetaDataByName = function(in_folderName, in_dataCache){
	...
	if (in_folderName in in_folderMetaDataMap){
		...
		RequestWithJWT({
			...
			//url: "https://www.googleapis.com/drive/v3/files?q=mimeType%3D'application%2Fvnd.google-apps.folder'+and+name%3D'" + name + "'",
			jwt: {
			...
			try {
				...
				//FOR NOW, if there are multiple folders of the same name, use the one with no parents (root folder?) else the first one
				//  better solution would be to specify the folder 'absolute' "project/release0/dev"
...
const getMetaData = function(in_id, in_dataCache){
...
const getChildrenOfFolder = function(in_folderID, in_dataCache){
...
*/
module.exports.getSpreadsheetWorksheet = function(in_spreadsheetID, in_worksheetName, in_dataCache){
